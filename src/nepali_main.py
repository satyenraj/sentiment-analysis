...
from config.nepali_config import NepaliConfig

def main():
    # Load configuration for Nepali
    config = NepaliConfig()
    
    # Create output directory with language identifier
    #output_dir = config.OUTPUT_DIR #os.path.join(config.BASE_OUTPUT_DIR, "nepali")
    output_dir = os.path.join(config.OUTPUT_DIR, "nepalBERT")
    ...
    train_df = train_df[train_df['language']=='nepali']
    val_df = val_df[val_df['language']=='nepali']
